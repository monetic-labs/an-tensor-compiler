...
impl InputBuffer {
    ...
    pub fn new(input_names: &[&str], feature_count: usize, device: &Device) -> Result<Self> {
        ...
        for (idx, name) in input_names.iter().enumerate() {
            ...
            let tensor = Tensor::zeros(&[1, 1], DType::F32, device).map_err(|e| {
                TensorCoreError::Tensor(format!("Failed to allocate tensor: {}", e))
            })?;
            tensors.push(tensor);
        }
...
impl BatchedInputBuffer {
    ...
    pub fn new(input_names: &[&str], batch_size: usize, device: &Device) -> Result<Self> {
        let names: Vec<String> = input_names.iter().map(|s| s.to_string()).collect();
        let name_to_col: HashMap<String, usize> = names
            .iter()
            .enumerate()
            .map(|(i, n)| (n.clone(), i))
            .collect();
    ...
    pub fn sync(&mut self) -> Result<()> {
        ...
        let tensor = Tensor::from_slice(
            &self.data,
            &[self.batch_size, self.feature_count],
            &self.device,
        )
