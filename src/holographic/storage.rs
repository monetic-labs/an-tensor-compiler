...
use crate::{Device, Result, Tensor, TensorCoreError};
...
impl TensorStore {
    ...
    pub fn save_organism(&self, organism: &OrganismTensor) -> Result<()> {
        ...
        std::fs::create_dir_all(&dir).map_err(TensorCoreError::Io)?;
        ...
        std::fs::write(&metadata_path, metadata_json).map_err(TensorCoreError::Io)?;

        // Save boundaries
        let boundaries_path = dir.join("boundaries.json");
        let boundaries_json = serde_json::to_string_pretty(&organism.boundaries)
            .map_err(|e| TensorCoreError::Serialization(e.to_string()))?;
        std::fs::write(&boundaries_path, boundaries_json).map_err(TensorCoreError::Io)?;
    ...
    fn save_context(&self, ctx: &BoundedContextTensor, base_dir: &Path) -> Result<()> {
        ...
        std::fs::create_dir_all(&ctx_dir).map_err(TensorCoreError::Io)?;
        ...
        let meta_json = serde_json::to_string_pretty(&ctx_meta)
            .map_err(|e| TensorCoreError::Serialization(e.to_string()))?;
        std::fs::write(&meta_path, meta_json).map_err(TensorCoreError::Io)?;
        ...
        let component_index: Vec<ComponentIndexEntry> = ctx
            .modules
            .iter()
            .enumerate()
            .flat_map(|(mod_idx, m)| {
                m.components
                    .iter()
                    .enumerate()
                    .map(move |(comp_idx, c)| ComponentIndexEntry {
                        id: c.id.full_path(),
        ...
        let index_json = serde_json::to_string_pretty(&component_index)
            .map_err(|e| TensorCoreError::Serialization(e.to_string()))?;
        std::fs::write(&index_path, index_json).map_err(TensorCoreError::Io)?;
    ...
    pub fn load_organism(&self, project_id: &str) -> Result<OrganismTensor> {
        ...
        if !dir.exists() {
            return Err(TensorCoreError::Config(format!(
                "Organism not found: {}",
                project_id
            )));
        }
        ...
        // Load metadata
        let metadata_path = dir.join("metadata.json");
        let metadata_json = std::fs::read_to_string(&metadata_path).map_err(TensorCoreError::Io)?;
        ...
        let boundaries: BoundaryRules = if boundaries_path.exists() {
            let json = std::fs::read_to_string(&boundaries_path).map_err(TensorCoreError::Io)?;
    ...
    fn load_context(&self, name: &str, base_dir: &Path) -> Result<BoundedContextTensor> {
        ...
        // Load metadata
        let meta_path = ctx_dir.join("metadata.json");
        let meta_json = std::fs::read_to_string(&meta_path).map_err(TensorCoreError::Io)?;
        ...
        let component_index: Vec<ComponentIndexEntry> = if index_path.exists() {
            let json = std::fs::read_to_string(&index_path).map_err(TensorCoreError::Io)?;
            serde_json::from_str(&json)
                .map_err(|e| TensorCoreError::Serialization(e.to_string()))?
        } else {
            Vec::new()
        ...
        for mod_idx in 0..meta.module_count {
            ...
            let module_hologram = module_holograms.get(&mod_key).cloned().unwrap_or_else(|| {
                Tensor::zeros((1,), candle_core::DType::F32, &self.device).unwrap()
            });
            ...
            if let Some(comp_entries) = components_by_module.get(&mod_idx) {
                for entry in comp_entries {
                    ...
                    let comp_hologram =
                        comp_holograms.get(&comp_key).cloned().unwrap_or_else(|| {
                            Tensor::zeros((1,), candle_core::DType::F32, &self.device).unwrap()
                        });
            ...
            let exports: Vec<ComponentId> = components
                .iter()
        ...
        let api: Vec<ComponentId> = modules
            .iter()
    ...
    fn load_named_tensors(&self, path: &Path) -> Result<HashMap<String, Tensor>> {
        let data = std::fs::read(path).map_err(TensorCoreError::Io)?;
    ...
    fn save_tensor(&self, tensor: &Tensor, path: &Path) -> Result<()> {
        ...
        let view = TensorView::new(tensor_data, shape, bytemuck::cast_slice(&data))
            .map_err(|e| TensorCoreError::Serialization(e.to_string()))?;
    ...
    pub fn list_organisms(&self) -> Result<Vec<String>> {
        ...
        for entry in std::fs::read_dir(&self.base_path).map_err(TensorCoreError::Io)? {
...
impl ComponentCache {
    ...
    pub fn hash_file(path: &Path) -> Result<String> {
        let content = std::fs::read(path).map_err(TensorCoreError::Io)?;
    ...
    pub fn save_components(
        ...
        std::fs::create_dir_all(&self.cache_dir).map_err(TensorCoreError::Io)?;

        // Save each component's hologram
        let hash_dir = self.cache_dir.join(&content_hash[..8]);
        std::fs::create_dir_all(&hash_dir).map_err(TensorCoreError::Io)?;

        let mut tensors_data: HashMap<String, Vec<f32>> = HashMap::new();
        let mut shapes: HashMap<String, Vec<usize>> = HashMap::new();
        let component_ids: Vec<String> = components.iter().map(|c| c.id.full_path()).collect();
    ...
    pub fn load_components(&self, content_hash: &str) -> Result<Vec<Tensor>> {
        let hash_dir = self
            .cache_dir
            .join(&content_hash[..8.min(content_hash.len())]);
        let tensor_path = hash_dir.join("tensors.safetensors");
        ...
        let data = std::fs::read(&tensor_path).map_err(TensorCoreError::Io)?;
        let tensors = SafeTensors::deserialize(&data)
            .map_err(|e| TensorCoreError::Serialization(e.to_string()))?;
    ...
    pub fn stats(&self) -> CacheStats {
        ...
        let total_components: usize = self.index.values().map(|e| e.component_ids.len()).sum();
    ...
    pub fn clear(&mut self) -> Result<()> {
        if self.cache_dir.exists() {
            std::fs::remove_dir_all(&self.cache_dir).map_err(TensorCoreError::Io)?;
        }
        self.index.clear();
        Ok(())
    }
    ...
    fn load_index(cache_dir: &Path) -> Result<HashMap<String, CacheEntry>> {
        ...
        let json = std::fs::read_to_string(&index_path).map_err(TensorCoreError::Io)?;
        serde_json::from_str(&json).map_err(|e| TensorCoreError::Serialization(e.to_string()))
    }
    ...
    fn save_index(&self) -> Result<()> {
        ...
        let json = serde_json::to_string_pretty(&self.index)
            .map_err(|e| TensorCoreError::Serialization(e.to_string()))?;
        std::fs::write(&index_path, json).map_err(TensorCoreError::Io)
    }
}
...
impl IncrementalState {
    ...
    pub fn save_commit(&self, root: &Path) -> Result<()> {
        ...
        std::fs::create_dir_all(&state_dir).map_err(TensorCoreError::Io)?;

        let commit_file = state_dir.join("last_encoded_commit");
        std::fs::write(&commit_file, &self.current_commit).map_err(TensorCoreError::Io)
    }
    ...
    fn load_previous_commit(root: &Path) -> Result<Option<String>> {
        ...
        if commit_file.exists() {
            let commit = std::fs::read_to_string(&commit_file).map_err(TensorCoreError::Io)?;
...
fn get_current_commit(root: &Path) -> Result<String> {
    let output = std::process::Command::new("git")
        ...
        .output()
        .map_err(|e| {
            TensorCoreError::Config(format!(
                "Git not available (required for incremental encoding): {}. \
             Install git or use full re-encoding instead.",
                e
            ))
        })?;
...
fn get_changed_files(root: &Path, since_commit: &str) -> Result<HashSet<PathBuf>> {
    let output = std::process::Command::new("git")
        ...
        .map_err(|e| {
            TensorCoreError::Config(format!(
                "Git diff failed (required for incremental encoding): {}",
                e
            ))
        })?;
