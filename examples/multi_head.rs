...
fn main() -> Result<()> {
    ...
    let spec = RuleSpec::parse("access_policy", r#"
    ...
    inputs.insert("low_risk".to_string(),
        Tensor::from_vec(vec![0.15f32], 1, &device)?);
    inputs.insert("all_checks_pass".to_string(),
        Tensor::from_vec(vec![0.95f32], 1, &device)?);
    inputs.insert("urgent".to_string(),
        Tensor::from_vec(vec![0.7f32], 1, &device)?);
    inputs.insert("needs_review".to_string(),
        Tensor::from_vec(vec![0.85f32], 1, &device)?);
